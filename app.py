...
NOW WITH JWT AUTHENTICATION - No more session token expiration!
"""
...
def get_jwt_access_token():
    ...
    try:
        # Try to load private key from environment variable first (Heroku)
        ...
        if response.status_code == 200:
            ...
            logger.error(f"JWT authentication failed: {response.status_code} - {response.text}")
            raise Exception(f"JWT auth failed: {response.text}")
